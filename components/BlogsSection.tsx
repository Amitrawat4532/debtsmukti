  ...
  return (
    <div className="w-full shrink-0">
        {!isHidden && (<>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-[24px] mt-10'>
                {blogs.slice(0, limit).map((blog) => ( 
                    <div className='flex flex-col' key={blog._id}>
                        <div className='relative w-full h-[200px] rounded-t-[24px] overflow-hidden'>
                        ...
                        <div className='bg-white rounded-[24px] md:p-10 p-5 -mt-5 z-10 relative border border-[#f1eded]  shadow-lg flex flex-col justify-between gap-4'>
                        <Link href={`/blogs/${blog.slug.current}`}>
                        <h3 className='md:text-[28px] text-[20px] font-semibold text-[#111]'>{blog.title}</h3>
                        </Link>
                        <p className='md:text-lg text-[16px] opacity-90 text-[#6B6969]'>{blog.excerpt}</p>
                    <p className='text-sm opacity-90 text-[#6B6969] text-right'>{formatDateWithOrdinal(blog.publishedAt || '')}</p>
                        </div>
                    </div>
                ))}
                </div>
