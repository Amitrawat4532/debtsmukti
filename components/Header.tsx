"use client";
import { siteData, navLinks, siteLogo } from "@/data";
import Image from "next/image";
import Link from "next/link";
import React, { useState } from "react";



// Social Icons as inline SVGs to avoid external dependencies
const InstagramIcon = () => (
  <svg
    width="28"
    height="28"
    viewBox="0 0 28 28"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <rect width="27.8969" height="27.8969" rx="13.9485" fill="#E3EFFF" />
    <path
      d="M16.6042 7.60742H11.2911C8.98325 7.60742 7.60742 8.98325 7.60742 11.2911V16.5978C7.60742 18.912 8.98325 20.2878 11.2911 20.2878H16.5978C18.9057 20.2878 20.2815 18.912 20.2815 16.6042V11.2911C20.2878 8.98325 18.912 7.60742 16.6042 7.60742ZM13.9476 16.4076C12.5908 16.4076 11.4876 15.3044 11.4876 13.9476C11.4876 12.5908 12.5908 11.4876 13.9476 11.4876C15.3044 11.4876 16.4076 12.5908 16.4076 13.9476C16.4076 15.3044 15.3044 16.4076 13.9476 16.4076ZM17.701 10.7014C17.6693 10.7775 17.6249 10.8473 17.5679 10.9107C17.5045 10.9677 17.4347 11.0121 17.3587 11.0438C17.2826 11.0755 17.2002 11.0945 17.1177 11.0945C16.9465 11.0945 16.788 11.0311 16.6676 10.9107C16.6105 10.8473 16.5661 10.7775 16.5344 10.7014C16.5027 10.6254 16.4837 10.5429 16.4837 10.4605C16.4837 10.3781 16.5027 10.2957 16.5344 10.2196C16.5661 10.1372 16.6105 10.0738 16.6676 10.0104C16.8134 9.86454 17.0353 9.79479 17.2382 9.83917C17.2826 9.84551 17.3206 9.8582 17.3587 9.87722C17.3967 9.8899 17.4347 9.90892 17.4728 9.93428C17.5045 9.9533 17.5362 9.985 17.5679 10.0104C17.6249 10.0738 17.6693 10.1372 17.701 10.2196C17.7327 10.2957 17.7518 10.3781 17.7518 10.4605C17.7518 10.5429 17.7327 10.6254 17.701 10.7014Z"
      fill="#0F2A47"
    />
  </svg>
);

const FacebookIcon = () => (
  <svg
    width="28"
    height="28"
    viewBox="0 0 28 28"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <rect width="27.8969" height="27.8969" rx="13.9485" fill="#E3EFFF" />
    <path
      d="M17.7525 7.60742H15.8505C15.0097 7.60742 14.2034 7.94141 13.6089 8.53592C13.0144 9.13043 12.6804 9.93676 12.6804 10.7775V12.6796H10.7783V15.2157H12.6804V20.2878H15.2165V15.2157H17.1185L17.7525 12.6796H15.2165V10.7775C15.2165 10.6094 15.2833 10.4481 15.4022 10.3292C15.5211 10.2103 15.6823 10.1435 15.8505 10.1435H17.7525V7.60742Z"
      fill="#0F2A47"
    />
  </svg>
);

const TwitterIcon = () => (
  <svg
    width="28"
    height="28"
    viewBox="0 0 28 28"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <rect width="27.8969" height="27.8969" rx="13.9485" fill="#E3EFFF" />
    <path
      d="M20.9221 8.24173C20.3149 8.66999 19.6427 8.99755 18.9313 9.21178C18.5494 8.77273 18.0419 8.46154 17.4775 8.3203C16.913 8.17906 16.3188 8.21459 15.7752 8.42208C15.2315 8.62956 14.7648 8.999 14.4379 9.48042C14.1111 9.96185 13.9401 10.532 13.9479 11.1138V11.7479C12.8337 11.7768 11.7296 11.5296 10.734 11.0285C9.73843 10.5274 8.88224 9.78787 8.24167 8.87575C8.24167 8.87575 5.70559 14.5819 11.4118 17.118C10.106 18.0044 8.55054 18.4488 6.97363 18.3861C12.6798 21.5562 19.654 18.3861 19.654 11.0948C19.6535 10.9182 19.6365 10.742 19.6033 10.5686C20.2504 9.93044 20.707 9.12474 20.9221 8.24173Z"
      fill="#0F2A47"
    />
  </svg>
);

const LinkedInIcon = () => (
  <svg
    width="28"
    height="28"
    viewBox="0 0 28 28"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <rect width="27.8969" height="27.8969" rx="13.9485" fill="#E3EFFF" />
    <path
      d="M16.4838 11.4121C17.4927 11.4121 18.4603 11.8129 19.1737 12.5263C19.8871 13.2397 20.2879 14.2073 20.2879 15.2162V19.6544H17.7519V15.2162C17.7519 14.8799 17.6183 14.5574 17.3805 14.3196C17.1426 14.0818 16.8201 13.9482 16.4838 13.9482C16.1475 13.9482 15.825 14.0818 15.5872 14.3196C15.3494 14.5574 15.2158 14.8799 15.2158 15.2162V19.6544H12.6797V15.2162C12.6797 14.2073 13.0805 13.2397 13.7939 12.5263C14.5073 11.8129 15.4749 11.4121 16.4838 11.4121Z"
      fill="#0F2A47"
    />
    <path
      d="M10.1435 12.0459H7.60742V19.6541H10.1435V12.0459Z"
      fill="#0F2A47"
    />
    <path
      d="M8.87546 10.1435C9.57578 10.1435 10.1435 9.57578 10.1435 8.87546C10.1435 8.17514 9.57578 7.60742 8.87546 7.60742C8.17514 7.60742 7.60742 8.17514 7.60742 8.87546C7.60742 9.57578 8.17514 10.1435 8.87546 10.1435Z"
      fill="#0F2A47"
    />
  </svg>
);

const socialLinks = [
  { icon: <InstagramIcon />, href: siteData.instagram, label: "Instagram" },
  { icon: <FacebookIcon />, href: siteData.facebook, label: "Facebook" },
  { icon: <TwitterIcon />, href: siteData.twitter, label: "Twitter" },
  { icon: <LinkedInIcon />, href: siteData.linkedin, label: "LinkedIn" },
];

const Header = () => {
  const [isOpen, setIsOpen] = useState(false);
  const toggleMenu = () => setIsOpen(!isOpen);

  return (
    <>
      {/* Top info bar — scrolls away normally */}
      <div className="bg-primary-blue">
        <div className="container px-5 py-4 flex justify-center md:justify-between items-center max-w-7xl mx-auto">
          <p className="text-white text-[16px] font-bold hidden md:block">{siteData.tagline}</p>
          <div className="flex items-center">
            <div className="flex items-center gap-2 border-r border-white pr-4">
              <svg width="16" height="16" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M8.725 11.375C8.16 11.375 7.565 11.24 6.95 10.98C6.35 10.725 5.745 10.375 5.155 9.95C4.57 9.52 4.005 9.04 3.47 8.515C2.94 7.98 2.46 7.415 2.035 6.835C1.605 6.235 1.26 5.635 1.015 5.055C0.755 4.435 0.625 3.835 0.625 3.27C0.625 2.88 0.695 2.51 0.83 2.165C0.97 1.81 1.195 1.48 1.5 1.195C1.885 0.815 2.325 0.625 2.795 0.625C2.99 0.625 3.19 0.67 3.36 0.75C3.555 0.84 3.72 0.975 3.84 1.155L5 2.79C5.105 2.935 5.185 3.075 5.24 3.215C5.305 3.365 5.34 3.515 5.34 3.66C5.34 3.85 5.285 4.035 5.18 4.21C5.105 4.345 4.99 4.49 4.845 4.635L4.505 4.99C4.51 5.005 4.515 5.015 4.52 5.025C4.58 5.13 4.7 5.31 4.93 5.58C5.175 5.86 5.405 6.115 5.635 6.35C5.93 6.64 6.175 6.87 6.405 7.06C6.69 7.3 6.875 7.42 6.985 7.475L6.975 7.5L7.34 7.14C7.495 6.985 7.645 6.87 7.79 6.795C8.065 6.625 8.415 6.595 8.765 6.74C8.895 6.795 9.035 6.87 9.185 6.975L10.845 8.155C11.03 8.28 11.165 8.44 11.245 8.63C11.32 8.82 11.355 8.995 11.355 9.17C11.355 9.41 11.3 9.65 11.195 9.875C11.09 10.1 10.96 10.295 10.795 10.475C10.51 10.79 10.2 11.015 9.84 11.16C9.495 11.3 9.12 11.375 8.725 11.375ZM2.795 1.375C2.52 1.375 2.265 1.495 2.02 1.735C1.79 1.95 1.63 2.185 1.53 2.44C1.425 2.7 1.375 2.975 1.375 3.27C1.375 3.735 1.485 4.24 1.705 4.76C1.93 5.29 2.245 5.84 2.645 6.39C3.045 6.94 3.5 7.475 4 7.98C4.5 8.475 5.04 8.935 5.595 9.34C6.135 9.735 6.69 10.055 7.24 10.285C8.095 10.65 8.895 10.735 9.555 10.46C9.81 10.355 10.035 10.195 10.24 9.965C10.355 9.84 10.445 9.705 10.52 9.545C10.58 9.42 10.61 9.29 10.61 9.16C10.61 9.08 10.595 9 10.555 8.91C10.54 8.88 10.51 8.825 10.415 8.76L8.755 7.58C8.655 7.51 8.565 7.46 8.48 7.425C8.37 7.38 8.325 7.335 8.155 7.44C8.055 7.49 7.965 7.565 7.865 7.665L7.485 8.04C7.29 8.23 6.99 8.275 6.76 8.19L6.625 8.13C6.42 8.02 6.18 7.85 5.915 7.625C5.675 7.42 5.415 7.18 5.1 6.87C4.855 6.62 4.61 6.355 4.355 6.06C4.12 5.785 3.95 5.55 3.845 5.355L3.785 5.205C3.755 5.09 3.745 5.025 3.745 4.955C3.745 4.775 3.81 4.615 3.935 4.49L4.31 4.1C4.41 4 4.485 3.905 4.535 3.82C4.575 3.755 4.59 3.7 4.59 3.65C4.59 3.61 4.575 3.55 4.55 3.49C4.515 3.41 4.46 3.32 4.39 3.225L3.23 1.585C3.18 1.515 3.12 1.465 3.045 1.43C2.965 1.395 2.88 1.375 2.795 1.375ZM6.975 7.505L6.895 7.845L7.03 7.495C7.005 7.49 6.985 7.495 6.975 7.505Z" fill="white"/>
              </svg>
              <a href={`tel:${siteData.phone}`} className="text-white text-[14px] md:text-[16px] font-bold">{siteData.phone}</a>
            </div>
            <div className="flex items-center gap-2 pl-4">
              <svg width="16" height="16" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="0.4" y="1.9" width="11.2" height="7.67059" rx="1.6" stroke="white" strokeWidth="0.8"/>
                <path d="M1.21094 1.90039H10.7891C11.2368 1.90059 11.5994 2.26316 11.5996 2.71094C11.5996 3.07082 11.3628 3.38768 11.0176 3.48926L7.58008 4.50098C6.54859 4.80433 5.45141 4.80433 4.41992 4.50098L0.982422 3.48926C0.637181 3.38768 0.400391 3.07082 0.400391 2.71094C0.400589 2.26316 0.763157 1.90059 1.21094 1.90039Z" stroke="white" strokeWidth="0.8"/>
              </svg>
              <a href={`mailto:${siteData.email}`} className="text-white text-[14px] md:text-[16px] font-bold">{siteData.email}</a>
            </div>
          </div>
          <div className="items-center gap-4 hidden md:flex">
            <Link href={siteData.instagram} 
                  aria-label="instagram"
            >
              <svg width="20" height="20" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M5.25751 0.649414H2.53617C1.3541 0.649414 0.649414 1.3541 0.649414 2.53617V5.25426C0.649414 6.43957 1.3541 7.14426 2.53617 7.14426H5.25426C6.43632 7.14426 7.14101 6.43957 7.14101 5.25751V2.53617C7.14426 1.3541 6.43957 0.649414 5.25751 0.649414ZM3.89684 5.15684C3.20189 5.15684 2.63684 4.59179 2.63684 3.89684C2.63684 3.20189 3.20189 2.63684 3.89684 2.63684C4.59179 2.63684 5.15684 3.20189 5.15684 3.89684C5.15684 4.59179 4.59179 5.15684 3.89684 5.15684ZM5.81931 2.23416C5.80307 2.27313 5.78034 2.30885 5.75111 2.34132C5.71864 2.37055 5.68292 2.39328 5.64395 2.40952C5.60498 2.42575 5.56276 2.4355 5.52055 2.4355C5.43287 2.4355 5.35168 2.40302 5.28998 2.34132C5.26075 2.30885 5.23802 2.27313 5.22179 2.23416C5.20555 2.19519 5.19581 2.15297 5.19581 2.11075C5.19581 2.06854 5.20555 2.02632 5.22179 1.98735C5.23802 1.94514 5.26075 1.91266 5.28998 1.88019C5.36467 1.8055 5.47833 1.76977 5.58225 1.79251C5.60498 1.79575 5.62446 1.80225 5.64395 1.81199C5.66343 1.81849 5.68292 1.82823 5.7024 1.84122C5.71864 1.85096 5.73488 1.8672 5.75111 1.88019C5.78034 1.91266 5.80307 1.94514 5.81931 1.98735C5.83555 2.02632 5.84529 2.06854 5.84529 2.11075C5.84529 2.15297 5.83555 2.19519 5.81931 2.23416Z" fill="white"/>
              </svg>
            </Link>
            <Link href={siteData.facebook}
                  aria-label="facebook"
            >
              <svg width="20" height="20" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clipPath="url(#clip0_42_1054)">
                  <path d="M5.8456 0.649414H4.87138C4.44074 0.649414 4.02774 0.820483 3.72324 1.12499C3.41873 1.42949 3.24766 1.84249 3.24766 2.27313V3.24735H2.27344V4.54632H3.24766V7.14426H4.54663V4.54632H5.52086L5.8456 3.24735H4.54663V2.27313C4.54663 2.187 4.58085 2.1044 4.64175 2.0435C4.70265 1.9826 4.78525 1.94838 4.87138 1.94838H5.8456V0.649414Z" fill="white"/>
                </g>
                <defs><clipPath id="clip0_42_1054"><rect width="7.79381" height="7.79381" fill="white"/></clipPath></defs>
              </svg>
            </Link>
            <Link href={siteData.twitter}
                  aria-label="twitter"
            >
              <svg width="20" height="20" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clipPath="url(#clip0_42_1056)">
                  <path d="M7.46953 0.973969C7.15855 1.19332 6.81423 1.3611 6.44983 1.47082C6.25426 1.24594 5.99433 1.08655 5.70521 1.01421C5.41609 0.941871 5.11173 0.960068 4.83329 1.06634C4.55485 1.17262 4.31577 1.36184 4.14838 1.60842C3.98099 1.855 3.89337 2.14705 3.89736 2.44505V2.76979C3.32667 2.78459 2.76118 2.65802 2.25124 2.40136C1.74131 2.14469 1.30277 1.7659 0.97468 1.29871C0.97468 1.29871 -0.324289 4.22139 2.59839 5.52036C1.92959 5.97434 1.13288 6.20197 0.325195 6.16985C3.24788 7.79356 6.82004 6.16985 6.82004 2.43531C6.81974 2.34485 6.81104 2.25462 6.79406 2.16577C7.12549 1.83892 7.35938 1.42624 7.46953 0.973969Z" fill="white"/>
                </g>
                <defs><clipPath id="clip0_42_1056"><rect width="7.79381" height="7.79381" fill="white"/></clipPath></defs>
              </svg>
            </Link>
            <Link href={siteData.linkedin}
                  aria-label="linkedin"
            >
              <svg width="20" height="20" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clipPath="url(#clip0_42_1058)">
                  <path d="M5.19552 2.59766C5.71229 2.59766 6.20788 2.80294 6.57329 3.16835C6.9387 3.53375 7.14398 4.02935 7.14398 4.54611V6.81931H5.84501V4.54611C5.84501 4.37386 5.77658 4.20866 5.65478 4.08686C5.53298 3.96505 5.36778 3.89663 5.19552 3.89663C5.02327 3.89663 4.85807 3.96505 4.73627 4.08686C4.61447 4.20866 4.54604 4.37386 4.54604 4.54611V6.81931H3.24707V4.54611C3.24707 4.02935 3.45235 3.53375 3.81776 3.16835C4.18317 2.80294 4.67876 2.59766 5.19552 2.59766V2.59766Z" fill="white"/>
                  <path d="M1.94838 2.92285H0.649414V6.81976H1.94838V2.92285Z" fill="white"/>
                  <path d="M1.2989 1.94838C1.6576 1.94838 1.94838 1.6576 1.94838 1.2989C1.94838 0.940198 1.6576 0.649414 1.2989 0.649414C0.940198 0.649414 0.649414 0.940198 0.649414 1.2989C0.649414 1.6576 0.940198 1.94838 1.2989 1.94838Z" fill="white"/>
                </g>
                <defs><clipPath id="clip0_42_1058"><rect width="7.79381" height="7.79381" fill="white"/></clipPath></defs>
              </svg>
            </Link>
          </div>
        </div>
      </div>

      {/* Sticky Nav — this is now a sibling, not a child, so sticky works correctly */}
      <header className="bg-white py-5 md:py-10 px-5 sticky top-0 z-50">
        <div className="flex items-center justify-between max-w-7xl mx-auto px-5">
          <Link href="/">
            <Image src={siteLogo.logoHeader} alt="logo" width={100} height={100} />
          </Link>

          {/* Desktop Nav */}
          <nav className="hidden md:flex items-center gap-12.5">
            {navLinks.map((link) => (
              <Link
                key={link.href + link.label}
                className="text-[14px] font-bold text-[#111111] hover:text-primary-blue transition-colors"
                href={link.href}
              >
                {link.label}
              </Link>
            ))}
            <Link
              href="/contact"
              className="bg-primary-blue text-white px-5 py-1.5 rounded-full text-[14px] font-bold hover:bg-opacity-90 transition-all"
            >
              Contact Us
            </Link>
          </nav>

          {/* Mobile Menu Button */}
          <button
            onClick={toggleMenu}
            className="md:hidden p-2 py-0 hover:bg-gray-100 rounded-lg transition-colors z-50 relative"
            aria-label={isOpen ? "Close navigation menu" : "Open navigation menu"}
          >
            {isOpen ? (
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M18 6L6 18M6 6L18 18" stroke="#0F2A47" strokeWidth="2.5" strokeLinecap="round"/>
              </svg>
            ) : (
              <svg width="36" height="24" viewBox="0 0 36 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M2.00001 24C1.43334 24 0.958673 23.808 0.576007 23.424C0.19334 23.04 0.00134023 22.5653 6.89654e-06 22C-0.00132644 21.4346 0.190674 20.96 0.576007 20.576C0.96134 20.192 1.43601 20 2.00001 20H34C34.5667 20 35.042 20.192 35.426 20.576C35.81 20.96 36.0013 21.4346 36 22C35.9987 22.5653 35.8067 23.0406 35.424 23.426C35.0413 23.8113 34.5667 24.0026 34 24H2.00001ZM2.00001 14C1.43334 14 0.958673 13.808 0.576007 13.424C0.19334 13.04 0.00134023 12.5653 6.89654e-06 12C-0.00132644 11.4347 0.190674 10.96 0.576007 10.576C0.96134 10.192 1.43601 9.99999 2.00001 9.99999H34C34.5667 9.99999 35.042 10.192 35.426 10.576C35.81 10.96 36.0013 11.4347 36 12C35.9987 12.5653 35.8067 13.0407 35.424 13.426C35.0413 13.8113 34.5667 14.0027 34 14H2.00001ZM2.00001 4C1.43334 4 0.958673 3.808 0.576007 3.424C0.19334 3.04 0.00134023 2.56533 6.89654e-06 2C-0.00132644 1.43467 0.190674 0.959999 0.576007 0.576C0.96134 0.192 1.43601 0 2.00001 0H34C34.5667 0 35.042 0.192 35.426 0.576C35.81 0.959999 36.0013 1.43467 36 2C35.9987 2.56533 35.8067 3.04066 35.424 3.426C35.0413 3.81133 34.5667 4.00266 34 4H2.00001Z" fill="#0F2A47"/>
              </svg>
            )}
          </button>
        </div>

        {/* Backdrop */}
        <div
          onClick={() => setIsOpen(false)}
          className={`md:hidden fixed inset-0 bg-black/40 z-40 transition-opacity duration-300 ${
            isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
          aria-label="Close navigation menu"
        />

        {/* Mobile Slide-in Panel */}
        <nav
          className={`md:hidden fixed top-0 right-0 h-screen p-8 w-full mobile-nav-bg  bg-white z-50 shadow-2xl flex flex-col justify-between  px-6 gap-2 transition-transform duration-300 ease-in-out ${
            isOpen ? "translate-x-0" : "translate-x-full"
          }`}
        >
          <div className="b flex flex-col gap-10">

          <button
          onClick={() => setIsOpen(false)}
          className="w-full flex justify-end"
          aria-label="Close navigation menu"
        ><svg width="24" height="24" viewBox="0 0 24 24" fill="#ffffff" xmlns="http://www.w3.org/2000/svg">
                <path d="M18 6L6 18M6 6L18 18" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"/>
              </svg></button>
                 <Link href="/" className="flex gap-5 items-center justify-center">
            <Image src={siteLogo.logoFooter} alt="logo" width={70} height={70} />
            <p className="text-[32px] font-bold text-white">Debt Mukti</p>
          </Link>
          </div>

          <div className=" flex flex-col gap-8">

          {navLinks.map((link) => (
            <Link
              key={link.href + link.label}
              className="font-bold max-w-76.25 w-full m-auto my-0  text-center text-[24px] text-white  py-3 border-b-[0.5px]  border-[#85C0FF] last:border-b-0"
              href={link.href}
              onClick={() => setIsOpen(false)}
            >
              {link.label}
            </Link>
          ))}
          </div>

          <div className=" w-full flex flex-col gap-10">

          <Link
            href="/contact"
            className="mt-4 bg-primary-light-blue text-black px-5 py-3 w-full rounded-full text-[14px] font-bold hover:bg-opacity-90 transition-all text-center"
            onClick={() => setIsOpen(false)}
          >
            Contact Us
          </Link>
            <div className="flex items-center gap-3 justify-center">
              {socialLinks.map((s) => (
                <a
                  key={s.label}
                  href={s.href}
                  aria-label={s.label}
                  className="w-8.5 h-8.5 rounded-full  flex items-center justify-center text-white/70 hover:text-white hover:border-white/60 transition-all duration-200"
                >
                  {s.icon}
                </a>
              ))}
            </div>
          </div>

        </nav>

      </header>
    </>
  );
};

export default Header;